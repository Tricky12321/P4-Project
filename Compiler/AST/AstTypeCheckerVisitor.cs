...
namespace Compiler.AST
{
	public class AstTypeCheckerVisitor : AstVisitorBase
	{
				...
				foreach (Tuple<VariableAttributeNode, string, AbstractNode> Attributes in node.Attributes)
				{
					...
					if (variableType != null)
					{
						if (isTypeCollection == isAttributeCollection)
						{
							...
							if (Attributes.Item3 is BoolComparisonNode)
							{
								expNode = (ExpressionNode)Attributes.Item3.Children[0];
							}
							else
							{
								expNode = (ExpressionNode)Attributes.Item3;
							}
		...
		public override void Visit(PushQueryNode node)
		{
			...
			ExpressionNode expNode;
			if (node.VariableToAdd is BoolComparisonNode)
			{
				expNode = (ExpressionNode)node.VariableToAdd.Children[0];
			}
			else
			{
				expNode = (ExpressionNode)node.VariableToAdd;
			}
		...
		public override void Visit(DeclarationNode node)
		{
			...
			if (node.Assignment != null)
			{
				...
				if (node.Assignment is BoolComparisonNode || node.Assignment is ExpressionNode)
				{
					ExpressionNode exprNode;
					if (node.Assignment is BoolComparisonNode)
					{
						exprNode = (ExpressionNode)node.Assignment.Children[0];
					}
					else
					{
						exprNode = (ExpressionNode)node.Assignment;
					}
		...
		public override void Visit(ReturnNode node)
		{
			...
			ExpressionNode expNode;
			if (node.LeftmostChild is BoolComparisonNode)
			{
				expNode = (ExpressionNode)node.LeftmostChild.Children[0];
			}
			else
			{
				expNode = (ExpressionNode)node.LeftmostChild;
			}
			if (expNode.ExpressionParts != null)
			{
		...
		public override void Visit(PrintQueryNode node)
		{
			...
			{
				ExpressionNode exp;
				if (abnode is BoolComparisonNode)
				{
					exp = (ExpressionNode)abnode.Children[0];
				}
				else
				{
					exp = abnode as ExpressionNode;
				}
